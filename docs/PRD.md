...
- ~~Structured Match Metadata Validation layer~~ -- **Implemented and merged**: `specs/013-match-metadata-validation/`, `src/cvip/metadata/` (`cvip validate`). Delivers exactly what this entry proposed -- an optional post-hoc stage aligning ball-by-ball metadata against Event Detection's output to report accuracy (`AccuracyReport`) and recover missed clips (`--recover`), plus dismissal-type/fielder enrichment (`--enrich`) beyond this entry's original scope. V1 ships one `MetadataProvider` (the ball-by-ball JSON shape proven in `ground_truth_v2/`), behind the same pluggable-provider interface this entry envisioned, so a future CricHeroes JSON/CricSheet CSV provider is a new implementation, not a redesign. Confirmed decoupled as designed: `cvip analyze`/`generate` are unaffected whether or not this feature is ever used.

...
- **Post-implementation performance fix**: this Assumption's "optimized for forward sequential traversal" was not actually true of the original implementation — `_retrieve_frame()` called `cap.set(CAP_PROP_POS_FRAMES, i)` before *every* read, including consecutive targets, which most OpenCV/FFmpeg backends treat as a genuine seek-to-nearest-keyframe-and-decode-forward even when `i` is exactly the next frame. This was invisible to this module's own benchmark (`tests/benchmark/test_scene_detection_performance.py`'s `multi_hour.mp4` fixture is a synthetic 320x240/10fps solid-black video, essentially free to decode regardless of seek pattern) but severe on real footage: measured at 7.5 fps on real 720p/30fps content via Scene Detection's `SamplingMode.FULL` consumer (specs/011-club-broadcast-overlay-support/'s real-video validation), extrapolating to ~159 minutes for a single 40-minute match — before any other pipeline stage. Fixed by tracking the last successfully retrieved frame index and skipping the redundant `.set()` call when the next target is exactly the following frame (restoring genuine sequential `.read()`-only decoding for that case); a non-sequential target still seeks explicitly, unchanged. Measured 3.4x throughput improvement (7.5 -> 25.6 fps) on the same real footage. This closes most, but not necessarily all, of the gap to the constitution's ≤40-minute/3-hour-match budget for `SamplingMode.FULL` consumers specifically — Scene Detection's own remaining per-frame cost (PySceneDetect's `ContentDetector`, or native decode cost itself) was not further profiled as part of this fix and may still need its own follow-up investigation.

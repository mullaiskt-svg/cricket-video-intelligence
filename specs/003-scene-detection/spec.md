...
- **Post-implementation amendment (real-video validation, specs/011-club-broadcast-overlay-support/'s investigation)**: this Assumption was never actually true. `video.scene_threshold` had been carrying PySceneDetect's own generic `ContentDetector` library default (`27.0`) since this feature's original implementation, never validated against real footage. Real-video testing found it produced **zero or near-zero boundaries** across representative samples of an actual club-cricket recording -- the underlying "how different do two consecutive frames look" score varies enormously within a single match (measured max ~5.7 during calm pre-match footage vs. ~95-97 during active play across three independent samples), and no single fixed `ContentDetector` threshold could be simultaneously correct for both regimes: a value low enough to catch cuts during calm footage produced hundreds of spurious boundaries once play started, while a value that behaved reasonably during play found almost nothing during calm footage.
  ...
  **Second independent match validated (WILD WANDERERS VS PHOENIX FIREHAWKS, different club/opponent, 1080p/`av1` source -- genuinely different resolution and codec lineage from the first match's 720p/`h264`)**: 5 targeted 2-minute windows (opening, early gameplay, middle overs, death overs, innings transition) using the same unchanged `min_content_val=8.0`, no retuning. Result: 0-4 boundaries per window, all `ORDINARY_CUT`, spacings of ~0.7-54s -- no repeat of the first match's death-overs tight-clustering pattern (closest pair here was 0.73s apart, not the 0.1-1.13s cluster seen before). Zero `REPLAY_TRANSITION` classifications across all 5 windows, but with only five 2-minute samples this cannot be distinguished from "no replay happened to fall inside these particular windows" (replays are sparse events, not uniformly distributed) -- not treated as a generalization failure on this evidence alone. Net: `min_content_val=8.0` continues to produce plausible, non-pathological boundary counts on a second, meaningfully different broadcast; still not enough evidence to either promote automatic per-video calibration or fully close the death-overs clustering question from the first match.

  **Separately discovered while preparing this second match's validation (out of scope for this feature, recorded here for continuity)**: the source video's original resolution (3840x2160/`vp9`, a different encode of the same match) made Scene Detection's per-frame cost impractically high -- measured 1.88 fps (vs. 25.6 fps on 720p), and a full-video run was killed after ~10 hours without completing. The 1080p/`av1` re-encode used for this validation measured 9-11 fps instead. See `docs/RISK_REGISTER.md` R3 for the full finding -- this platform currently has no defined behavior for rejecting or downsampling unreasonably high-resolution source video before Scene Detection.

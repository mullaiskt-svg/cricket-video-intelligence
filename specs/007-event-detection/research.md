...
**SUPERSEDED by `specs/015-innings-transition-detection`**: real-match validation disproved this decision's central assumption. A single noisy OCR frame satisfying "runs and wickets both dropped" was sufficient to fire a false transition, and the `over_number`-reset check this decision rejected as "unnecessary" would have caught it for free — the real incident's two false transitions were never independently cross-checked against over/ball at all. This module's own innings-transition logic (`self._innings`/`_process_comparison`'s inline check) is replaced by a single shared `InningsTracker` (`src/cvip/video/innings_transition.py`) that both this module and `scoreboard_ocr.py`/`orchestrator.py` now consume identically — see that feature's research.md for the full corrected design (multi-signal evidence, persistence requirement, protected baseline, hard segment-count bound).

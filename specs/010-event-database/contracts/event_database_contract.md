...
**Postconditions**: every element is inserted via one batched `executemany()` call inside a single transaction (all-or-nothing per batch — a mid-batch failure rolls back that call's transaction before the `EventDatabaseError`/underlying exception propagates, so no partial rows from a failed batch survive even a *later*, unrelated commit on the same connection — PR #14 review finding, since `sqlite3` otherwise leaves a failed batch's transaction open rather than discarding it), preserving every field exactly and `scoreboard_readings`' own `timestamp_seconds` order (FR-008, FR-009, FR-010). An empty sequence is a valid no-op — zero rows inserted, no error (Edge Cases). `events.clip_start_seconds`/`clip_end_seconds` are always `NULL` immediately after `persist_events` (FR-010). Each call emits one diagnostics record (FR-020).

### `db.update_clip_window(event_key: str, clip_start_seconds: float, clip_end_seconds: float) -> None`

**Postconditions**: updates only `clip_start_seconds`/`clip_end_seconds` on the `events` row whose `event_id = int(event_key)` — every other column of that row is untouched (FR-011). **Not** subject to the `WRITE_AGAINST_COMPLETED_MATCH` gate (corrected post-review — PR #14; this contract originally, incorrectly, stated the opposite): this call represents `cvip generate`-time bookkeeping, which only ever runs *after* `cvip analyze` has already marked the match `COMPLETE` — gating it the same way as the batch-persist methods would make the platform's normal analyze-once-generate-later workflow permanently unusable. Repeated calls for the same `event_key` each succeed and reflect only the most recent call (Edge Cases; Assumptions — best-effort tracking, not `generate`'s correctness mechanism).

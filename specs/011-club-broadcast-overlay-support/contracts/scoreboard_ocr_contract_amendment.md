...
- A reading that fits **neither** format (no compound-score token found, and the original regexes also don't match) still resolves through the original code path and therefore still ends in `PLAYER_PARSE_FAILED` exactly as before (FR-008) — this amendment does not add a third, more permissive fallback.

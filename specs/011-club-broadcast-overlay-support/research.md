...
## Decision 10 (post-implementation amendment): `batter` no longer gates the score fields in `_validate_reading()`

**Decision**: `_validate_reading()` (`specs/005-scoreboard-ocr/spec.md` FR-030's original home) no longer unconditionally returns `PLAYER_PARSE_FAILED` when `batter` is `None`. Instead, it checks whether *any* of `runs`/`wickets`/`over_number`/`ball_in_over` is present; only when none of them are present, and `batter` is also absent, does `PLAYER_PARSE_FAILED` fire. Whenever at least one score field is present, validation proceeds straight to the existing monotonic-rule checks (unchanged) — a reading whose score fields are internally consistent is accepted and advances `_LastAcceptedReading`, regardless of whether `batter` was locatable. `batter`/`non_striker`/`bowler` remain `None` on the resulting `ScoreboardSample` exactly as before when unlocatable.

**Rationale**: Discovered via this feature's own quickstart.md Steps 3/5, run against the real First8Overs.mp4 recording after initial implementation. Diagnostics showed `parse_confidence_zero_count = 1588/2383` (67%) on the full run, almost entirely `PLAYER_PARSE_FAILED`. A timeline dump of every accepted reading's `over_number`/`ball_in_over`/`runs` confirmed the consequence: readings were skipping 1-5 balls between consecutive accepted entries, because a reading with a perfectly valid score but an unlocatable name was being discarded *in its entirety* rather than just losing its name field. Event Detection's FOUR/SIX rule (`specs/007-event-detection/spec.md`) requires a strict single-ball advance between accepted readings — so a boundary hit inside one of these gaps was silently dropped, not misclassified, just never evaluated. A full pipeline run before this fix detected only 6 events (3 FOUR, 3 WICKET) from an 8-over innings, implausibly few; WICKET events were comparatively unaffected because that rule doesn't require single-ball-advance.

This targets the actual bottleneck directly: real-footage name legibility is *worse* than score legibility (the score is large, high-contrast digits; a player name is small text easily lost to compression), so gating the more-reliable signal on the less-reliable one was backwards for this use case. The fix is deliberately narrow — `batter`'s absence still matters (it's simply `None` on the sample, and `PLAYER_PARSE_FAILED` still exists for the genuinely-nothing-parsed case) — this is a validation-logic decoupling, not a relaxation of what counts as a valid score.

**Alternatives considered**:
- *Relax Event Detection's single-ball-advance requirement instead* (let FOUR/SIX fire on any `runs_delta` of 4/6 regardless of ball spacing) — rejected: this trades missed events for possibly-wrong ones (e.g. two separate 2-run deliveries summing to 4 across a gap would be misclassified as a single FOUR), and that rule lives in an unrelated, already-merged module (`specs/007-event-detection/`) with its own spec rationale for requiring consecutive balls.
- *Improve the name-extraction heuristic itself (better ROI, more aggressive preprocessing) to reduce the failure rate at the source* — valuable, but a separate, `Out of Scope`-adjacent concern (this feature's own Out of Scope section already excludes "improving raw OCR character-recognition accuracy"); it would reduce, not eliminate, the gap problem, whereas decoupling validation eliminates the *coupling* that turns a name-read failure into a score-read failure.
- *Leave `PLAYER_PARSE_FAILED`'s trigger condition alone and instead have `_process_frame()` update the baseline from a rejected reading's fields anyway* — rejected as more surprising: it would create samples reporting `parse_confidence = 0.0` (implying "don't trust this") while silently still using its values internally, an inconsistency a future reader of `OCREvidence` would find confusing. Decoupling at the validation-result level keeps `parse_confidence` an honest signal of "the score fields in *this* sample are usable."

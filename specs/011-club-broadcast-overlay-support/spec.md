...
- **FR-013 (post-implementation amendment, full-match validation)**: System MUST reject a reading whose `over_number` falls outside a generous sanity ceiling (`OVER_NUMBER_MAX = 50`), the same way `ball_in_over`/`wickets` are already bounded. Discovered against the full PLATINUM CUP FINAL recording (3h10m, cross-validated against official CricHeroes commentary and scorecard): a single garbled OCR frame produced `over_number = 600` (a misread of unrelated on-screen text coincidentally matching the generic parser's `over.ball` token shape) that passed every existing check -- it wasn't a decrease, and nothing bounded its absolute magnitude -- and became the new accepted-reading baseline. Every genuine subsequent reading for the rest of that innings (~74 minutes, roughly overs 6 through 20) was then rejected as `INVALID_OVER_SEQUENCE` against the corrupted baseline, entirely blacking out that stretch of the timeline.
- **FR-014 (post-implementation amendment, full-match validation)**: `_LastAcceptedReading`'s `over_number`/`ball_in_over` fields MUST NOT advance from a reading that lacks accompanying `runs` and `wickets` values. A reading with `over_number` present but `runs`/`wickets` both absent can only have come from the generic parser's standalone `over.ball` token regex matching in isolation (the compound-score parser always produces all four numeric fields atomically or none at all) -- a weaker signal with no score context to corroborate it. Without this gate, one such spurious match silently advanced `over_number` ahead of where the game actually was, and every genuinely correct subsequent reading was rejected as an over-number decrease against that now-inflated baseline -- the same failure mode FR-013 addresses, for values too small for that ceiling to catch.
- **FR-015 (post-implementation amendment, full-match validation)**: A reading whose `over_number` and `ball_in_over` both exactly match the accepted-reading baseline (describing the same historical ball) MUST have its `runs` value match the baseline too, or be rejected. Discovered: a single-digit OCR misread within an otherwise fully atomic compound-score match turned a genuine `"31-3/6.1(20)"` into `"311-3/6.1(20)"` -- since `over_number`/`ball_in_over` were unchanged, it was neither a decrease nor a sequence violation, and the corrupted value became the new baseline. An earlier version of this fix used an absolute magnitude cap on the runs increase instead; that version was found to also reject a legitimate large catch-up jump when FR-014 of `specs/005-scoreboard-ocr/spec.md` (the innings-transition heuristic) occasionally misfires on an unrelated garbled reading and temporarily resets the baseline low -- the same-ball comparison used here cannot conflict with that recovery, since a genuine catch-up jump always comes with the over/ball having also advanced.

**Cumulative real-video validation impact** (FR-012 through FR-015, measured against the official CricHeroes ball-by-ball commentary and scorecard for the PLATINUM CUP FINAL recording, cross-validated across three independent sources that agree exactly with each other): boundary/wicket recall rose from 26/45 (58%) before FR-013-FR-015 to 41/45 (91%) after all four fixes. The four remaining misses (one wicket, one six, two fours) were not further root-caused as part of this pass.

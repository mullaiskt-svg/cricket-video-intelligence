...
2. Resolve `output_db_path` (`--output-db`, or the `file_hash[:12]`-derived default) and create its parent directory if missing (corrected post-review, PR #15 — the default `data/matches/` subdirectory does not exist on a fresh checkout; only `data/.gitkeep` is checked in, so opening the default path would otherwise fail at SQLite's own file-open step). Open the Event Database there. Call `check_analysis_status(file_hash)`. If `COMPLETE` or `IN_PROGRESS` and `request.force` is `False` → `OrchestratorError(ALREADY_ANALYZED)`, no further stage runs (FR-002). If `request.force` is `True` and a prior record exists, call `reset_for_forced_reanalysis(file_hash)` (FR-003).
...
An `EventDatabaseError` raised by step 2 itself (`open_database()`'s own corruption/schema-mismatch checks, or `check_analysis_status()`/`reset_for_forced_reanalysis()`/`begin_analysis()`) is also caught and translated to `OrchestratorError(DATABASE_FAILURE)` (corrected post-review, PR #15 — this previously escaped unwrapped, past `cli.py`'s own error handling, falling back to the generic `GENERAL_FAILURE`/exit-1 safety net instead of the correct database-failure exit code 7). No `fail_analysis()` call for this case: by construction, every one of these calls either runs before an `IN_PROGRESS` record could exist yet, or *is* the reason that record's state can't be established cleanly in the first place.
...
4. Build a `ClipGenerationRequest` (Clip Generator's own existing type) directly from the query result (`QueriedEvent` is already structurally compatible, per Event Database's own FR-013) plus `request.include_replays`/`pre_roll_seconds`/`post_roll_seconds`/`merge_gap_seconds`. `source_video_path` is read back from `get_match_summary().source_video_path` on the same open connection (corrected post-review, PR #15 — `request.match_id` is a database identifier, e.g. `"match_001"`, not a real file path; the actual source video path was already persisted during `analyze()`'s `begin_analysis()` call). Run Clip Generator (`generate_clips`).
5. If `plan.total_clips == 0` (Edge Cases: an empty filter result, or every matching event replay-excluded), return a `GenerateResult` with `clip_count=0`/`event_count=len(queried_events)` immediately — never construct a `StitchRequest` (corrected post-review, PR #15 — Video Stitcher's own validation rejects an empty `ClipPlan` as `EMPTY_CLIP_PLAN`, which would otherwise turn this documented valid outcome into a spurious `EXPORT_FAILURE`). Otherwise build a `StitchRequest` from the resulting `ClipPlan` and `request.output_path`. Run Video Stitcher (`stitch_video`). On `VideoStitchingFailureReason.MISSING_FFMPEG` → `OrchestratorError(MISSING_NATIVE_DEPENDENCY)`; any other reason → `OrchestratorError(EXPORT_FAILURE)`.

...
- **Known limitation, deferred to real-data validation (PR review finding)**: the nearest-before search (research.md Decision 1) has no *minimum* distance floor — only the `max_cut_search_seconds` ceiling. Because the event timestamp is the scoreboard-*update* time, which lags real play by ~15s (`state_transition.py`), a cut located only a second or two before the event timestamp would snap the clip to a point *after* the delivery it means to show, discarding the fixed pre-roll's lag compensation entirely. This is not guarded today, deliberately: (a) it is currently dormant — `orchestrator.generate()` does not populate `scene_cuts` (research.md Decision 2), so no real run exercises it; and (b) whether a minimum floor is needed, and what value it should take, is exactly the kind of snapping-calibration question research.md Decisions 3 and 5 explicitly defer to the quickstart.md real-data validation (which cannot run until the `ww_vs_pf` scene-boundary fixture and production wiring both exist). Inventing a floor constant now, ahead of that evidence, would be the premature calibration this platform's constitution warns against. Revisit together with the max-search-distance calibration when the feature is wired and validated.

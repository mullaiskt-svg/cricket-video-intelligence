...
### Module 5: Event Detection
- Input: the cleaned scoreboard timeline (`CleanedScoreboardSample` sequence) from Module 4a, **plus** Module 4's own raw `ScoreboardOcrResult` (for confidence lookup by timestamp — see Confidence derivation below), plus the replay timeline from Module 3 (Replay Detection). Diffing happens against 4a's cleaned values only (per its contract, `specs/006-ocr-timeline-smoother/contracts/ocr_timeline_smoother_contract.md`) — Module 5 MUST NOT diff Module 4's raw output directly or implement its own gap-filling/outlier logic; the raw result is consulted only for its `ocr_confidence`/`parse_confidence` fields, never its values.
- Logic: Compare consecutive cleaned readings. `CleanedScoreboardSample` carries no confidence fields (Module 4a's contract: reliability filtering already happened upstream), so instead of a per-reading confidence skip, any comparison where either bracketing sample has a `null` core field (still possible during a leading gap with no known-good reading yet) is simply skipped — there is nothing to diff.
- Initial MVP event types: `FOUR`, `SIX`, `WICKET`, `TEAM_MILESTONE` — this is a hard ceiling, not a starting point: see "Event Taxonomy & Detectability" below for why dismissal-subtype events (`RUN_OUT`, `CAUGHT`, etc.), `HAT_TRICK`, `MATCH_WINNING_SHOT`, and (for a distinct data-source reason) `FIFTY`/`CENTURY` are explicitly out of scope until a new data source is designed for them. `WICKET` stays generic (no subtype) for MVP and V1.5.
- Output: one row per detected event, inserted into the `events` table (see Database Schema; collectively this is the "Event Database"). Module 5 populates `event_type`, `timestamp_seconds`, `innings`, `over_number`, `ball_in_over`, `player`, `confidence`, `importance` (per Module 7's scoring), `milestone_value` (`TEAM_MILESTONE` rows only), and `is_replay` (cross-referenced against Module 3's replay timeline). It leaves `team` `NULL` for MVP (no module extracts a team name, `specs/007-event-detection/spec.md` FR-012). It does **not** populate `clip_start_seconds`/`clip_end_seconds` — those are computed later by Module 8 (Clip Generator) applying the 8s/12s pre/post-roll window, so a fresh event row has them `NULL` until Module 8 runs.
- **Processing model & precedence**: internally, Module 5 runs each comparison through a fixed stage order (Timeline Comparison → Event Rule Engine → Replay Annotation → Confidence Assignment → Importance Assignment) and a precedence-ordered rule engine (innings-transition heuristic, then `WICKET`, then `FOUR`/`SIX` — mutually exclusive; `TEAM_MILESTONE` is orthogonal and may co-occur with any of them) — see `specs/007-event-detection/spec.md` Processing Model and FR-023 for the full model, including the internal `EventEvidence` diagnostics record and each event's deterministic `event_key`.
- **Confidence derivation**: `events.confidence` is the minimum of `ocr_confidence` and `parse_confidence` across Module 4's **raw** `scoreboard_readings` row(s), looked up by timestamp, that bracket the detected delta (i.e., the lower of the two raw readings bracketing the change, and the lower of each raw reading's own two confidence fields). A confidence chain is only as strong as its weakest link — if either raw reading was noisy or its parse was rule-inconsistent, the derived event's confidence must reflect that, not average it away, even if Module 4a's smoothing already resolved a clean *value* for that timestamp. (Revised during Module 5's own `/speckit-specify`, `specs/007-event-detection/`: the original wording assumed confidence fields would still be present on the smoothed sequence itself, which Module 4a's finalized contract intentionally does not carry.)
- **`player` resolution**: resolved during Module 5's own `/speckit-specify` (`specs/007-event-detection/`, clarified 2026-07-29) as the dismissed batter only, not the bowler — the `events` table has a single `player` column, and highlight filtering by player is most naturally batter-centric for a wicket moment. `FOUR`/`SIX`/`TEAM_MILESTONE` events leave `player` unpopulated (`NULL`), since no per-batter running score exists to attribute a boundary or milestone to an individual (see "Event Taxonomy & Detectability" above).
- **`TEAM_MILESTONE` threshold**: every 50 runs by default, configurable via `config/default.yaml`'s `events.team_milestone_interval` (resolved alongside `player`, same clarification session).
...
**Canonical MVP/V1.5 event set**: `FOUR`, `SIX`, `WICKET` (generic, no subtype), `TEAM_MILESTONE`. Everything else in PRD Section 8's taxonomy — including `FIFTY`/`CENTURY` — remains valid long-term product vision, gated behind the data-source work above.
...
CREATE TABLE events (
  ...
  milestone_value INTEGER,  -- the specific threshold reached (e.g. 50, 100, 150); NULL except
                             -- for TEAM_MILESTONE rows (specs/007-event-detection/spec.md FR-026)
